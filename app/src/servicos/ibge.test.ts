...
import {lerTabela, procurar} from './ibge';
...
function linha(cep: string, rua: string, numero: number, lat: number, lng: number, comp = '') {
  const c = new Array(29).fill('');
  c[8] = cep; c[9] = 'ARUANA'; c[10] = 'RUA'; c[12] = rua; c[13] = String(numero);
