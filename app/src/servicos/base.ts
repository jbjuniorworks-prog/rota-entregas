...
import {chaveRua, mesmaRua, normal, palavrasRua, tipoDaRua} from '../logica/texto';
...
export function escolherTrecho(trechos: Trecho[], cidade: string, perto: Ponto | null, tipo = ''): Trecho | null {
  ...
  const lista = doTipo.length ? doTipo : naCidade;
...
export async function ruaNaBase(rua: string, cidade: string, perto: Ponto | null): Promise<Candidato | null> {
  ...
  try {
    const campos = 'nome, tipo, cidade, lat, lng, linha';
    ...
    if (!data || !data.length) {
      ...
      data = (parecidas.data || []).filter((t: any) => t.nome && mesmaRua(rua, t.nome));
      if (!data.length) return null;
    }
    const t = escolherTrecho(data as Trecho[], cidade, perto, tipoDaRua(rua));
    if (!t) return null;
    ...
    return {...p, precisao: 'rua', rua: t.nome || rua, fonte: 'nossa base', exibido: `${titulo} — ${t.cidade} (pela nossa base de ruas)`};
  } catch {
    return null;
  }
}
